...
def train():
    ...
    ax.legend(); ax.set_xlabel("Step"); ax.set_ylabel("Loss"); ax.set_title("Training Progress")
    if use_interactive_backend:
        fig.canvas.draw()
    plt.tight_layout()
    ...
    # === CTC: taruh blank di indeks terakhir ===
    blank_id = vocab_size_ctc
    ...
    scheduler_name = (scheduler_cfg.get("name") or config.get("lr_scheduler_type", "")).lower()
    ...
    loss_warmup_steps = config["loss_weights"]["warmup_steps"]
    ...
    accum_batches = 0
    accum_loss = accum_vocab_loss = accum_token_loss = 0.0
    ...
    for batch in train_loader:
        ...
        phoneme_lengths = torch.tensor(input_lengths, dtype=torch.long, device=device)
        mask = length_to_mask(phoneme_lengths).to(device)             # True=valid
        attn_mask = mask.int()                                        # 1=valid ✅
        ...
        with torch.amp.autocast(device_type=device_type, enabled=config["mixed_precision"]):
            ...
            # Siapkan target: buang PAD/EOS
            valid_mask = (words != pad_id)
            if eos_id is not None:
                valid_mask &= (words != eos_id)
            targets = torch.masked_select(words, valid_mask)          # concat all

            # === KOREKSI TERBESAR: panjang input per-sampel ===
            input_lengths_tensor = phoneme_lengths                    # (N,)
            target_lengths_tensor = torch.tensor(token_lengths, dtype=torch.long, device=device)

            # Skip batch jika ada target kosong (aman)
            if torch.any(target_lengths_tensor == 0) or targets.numel() == 0:
                optimizer.zero_grad(set_to_none=True)
                continue

            # Pastikan dimensi output menyertakan kelas blank di indeks terakhir
            # (C seharusnya == vocab_size_ctc+1)
            assert C == ctc_output_dim, f"CTC head dim {C} != {ctc_output_dim}"

            loss_vocab = ctc_loss_fn(log_probs, targets, input_lengths_tensor, target_lengths_tensor)

            # Token-level CE pada posisi masked (jika ada)
            ...
            for pred, lbl, length, masked in zip(tokens_pred, labels, input_lengths, masked_indices):
                if len(masked) > 0:
                    span = lbl[:length][torch.tensor(masked, dtype=torch.long, device=device)]
                    if span.numel() > 0:
                        loss_token += F.cross_entropy(pred[:length][masked], span)
                        count += 1
            if count > 0:
                loss_token /= count

            if global_step >= loss_warmup_steps:
        ...
        accum_vocab_loss += float(loss_vocab.detach())
        accum_token_loss += float(loss_token.detach())
        ...
        if wandb_cfg.get("enabled", False):
            wandb.log({"step": global_step, "loss": avg_loss, "vocab_loss": avg_vocab_loss, "token_loss": avg_token_loss, "lr": lr})

        if global_step % config["plot"]["refresh_interval"] == 0:
            steps.append(global_step); losses.append(avg_loss)
            vocab_losses.append(avg_vocab_loss); token_losses.append(avg_token_loss)
            line1.set_data(steps, losses); line2.set_data(steps, vocab_losses); line3.set_data(steps, token_losses)
